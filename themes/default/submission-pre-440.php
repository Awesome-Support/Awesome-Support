...
<div class="wpas wpas-submit-ticket">
	...
	<form class="wpas-form NEW" role="form" method="post" action="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" id="wpas-new-ticket" enctype="multipart/form-data">
		...
		<?php
		/**
		 * The mumei_ayuda_submission_form_inside_before has to be placed
		 * inside the form, right in between the form opening tag
		 * and the subject field.
		 *
		 * @since  3.0.0
		 */
		do_action( 'mumei_ayuda_submission_form_inside_before_subject' );
		...
		$subject = new MUMEI_AYUDA_Custom_Field( 'title', $subject_args );
		echo wp_kses($subject->get_output(), get_allowed_html_wp_notifications());		

		/**
		 * The mumei_ayuda_submission_form_inside_after_subject hook has to be placed
		 ...
		 * @since  3.0.0
		 */
		do_action( 'mumei_ayuda_submission_form_inside_after_subject' );
		...
		$body = new MUMEI_AYUDA_Custom_Field( 'message', $body_args );
