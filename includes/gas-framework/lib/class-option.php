...
class GASFrameworkOption {
	...
	public static function factory( $settings, $owner ) {
		...
		$className = 'GASFrameworkOption' . str_replace( ' ', '', ucwords( str_replace( '-', ' ', $settings['type'] ) ) );
		...
		if ( ! class_exists( $className ) && ! class_exists( $settings['type'] ) ) {
			...
			$x_content = __( 'Option type or extended class %s does not exist.', 'awesome-support' );
	...
	function __construct( $settings, $owner ) {
		...
		$this->type = is_a( $owner, 'GASFrameworkMetaBox' ) ? self::TYPE_META : self::TYPE_ADMIN;
		$this->type = is_a( $owner, 'GASFrameworkCustomizer' ) ? self::TYPE_CUSTOMIZER : $this->type;
	...
	protected function getFramework() {
		if ( is_a( $this->owner, 'GASFrameworkAdminTab' ) ) {
