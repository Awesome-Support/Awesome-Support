...
class GASFramework {
  ...
  private $blogId;
	 ...
	 *
	 * @var array of GASFrameworkAdminPage, GASFrameworkMetaBox, & GASFrameworkCustomizer
	 */
	private $mainContainers = array();

	/**
	 * All Google Font options used. This is for enqueuing Google Fonts for the frontend
	 * TODO Move this to the GASFrameworkOptionSelectGooglefont class and let it enqueue from there
	 *
	 * @var array GASFrameworkOptionSelectGooglefont
	 */
	 ...
	 *
	 * @var array of GASFrameworkOption
	 */
	...
	private $defaultSettings = array(
		'css' => 'generate', 	// If 'generate', GAS will try and generate a cacheable
	...
	public static function getInstance( $optionNamespace ) {
		...
		$newInstance = new GASFramework( $optionNamespace );
	...
	/**
	 * Gets all active instances of GAS Framework
	 *
	 * @since 1.9.2
	 *
	 * @return array An array of GASFramework objects
	 */
	...
	/**
	 * Creates a new GASFramework object
	 *
	 * @since 1.0
	 *
	 * @param string $optionNamespace The namespace to get options from.
    ...
    $this->blogId = get_current_blog_id();
		...
		$this->cssInstance = new GASFrameworkCSS( $this );
	 ...
	 * @since 1.2.1
	 *
	 * @param GASFrameworkOption $option The option that was just created.
	 *
	 * @return void
	 */
	public function rememberAllOptions( $option ) {
		if ( ! empty( $option->settings['id'] ) ) {		
			...
			if ( is_admin() && isset( $this->optionsUsed[ $option->settings['id'] ] ) ) {
				...
				$x_content = __( 'All option IDs per namespace must be unique. The id %s has been used multiple times.',  'ayuda-help-desk'  );
	...
	/**
	 * Loads all the admin scripts used by GAS Framework
	 *
	 * @since 1.0
	 *
	...
	public function loadAdminScripts( $hook ) {
		...
		// Only enqueue scripts if we're on a GAS options page.
		if ( in_array( $hook, $panel_ids ) || ! empty( $this->mainContainers['meta-box'] ) ) {
			wp_enqueue_media();
			wp_enqueue_script( 'tf-serialize', GASFramework::getURL( '../js/min/serialize-min.js', __FILE__ ) );
			wp_enqueue_script( 'tf-styling', GASFramework::getURL( '../js/min/admin-styling-min.js', __FILE__ ) );
			wp_enqueue_style( 'tf-admin-styles', GASFramework::getURL( '../css/admin-styles.css', __FILE__ ) );
		}
	}
	 ...
	 *
	 * @see GASFrameworkOption->getValue()
	 */
	 ...
	 *
	 * @see GASFrameworkOption->setValue()
	 */
	 ...
	 *
	 * @return GASFrameworkAdminPage The created admin page
	 */
   ...
   *
 	 ...
 	 *
	 *	@return GASFrameworkAdminPage The created sample coennt page.
	 */
	 ...
	 *
	 * @return GASFrameworkMetaBox The created meta box
	 */
	 ...
	 *
	 * @return GASFrameworkCustomizer The created section
	 */
	 ...
	 *
	 * @return GASFrameworkCustomizer|GASFrameworkAdminPage|GASFrameworkMetaBox The created container
	 */
	public function createContainer( $settings ) {
		if ( empty( $settings['type'] ) ) {
			...
			$x_content = __( '%1$s needs a %2$s parameter.',  'ayuda-help-desk'  );
		...
		$class = 'GASFramework' . str_replace( ' ', '', ucfirst( str_replace( '-', ' ', $settings['type'] ) ) );
		...
		if ( ! class_exists( $class ) ) {
			...
			$x_content = __( 'Container of type %s, does not exist.',  'ayuda-help-desk'  );
	 ...
	 * adding an option (if the option id was used in $gas->removeOption).
	 *
	 * @since 1.2.1
	 * @access public
	 *
	 ...
	 * a theme_mod option. It further checks whether these are GAS customizer options,
	...
	public function addCustomizerSaveFilter( $value, $optionName, $oldValue ) {
		...
		// Checks whether a Gas customizer is in place.
