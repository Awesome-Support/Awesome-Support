...
class GASFrameworkOptionFile extends GASFrameworkOption {
    ...
    /**
     * Generates CSS for the font, this is used in GASFrameworkCSS.
     *
     * @param	String               $css The CSS generated.
     * @param	GASFrameworkOption $option The current option being processed.
    ...
    public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
        $wp_customize->add_control( new GASFrameworkOptionFileUploadControl( $wp_customize, $this->getID(), array(
        ...
        <script>
            jQuery(document).ready(function($){
                ...
                $('body').on('click', '.tf-file-upload', function(event) {
                    ...
                    var frame = wp.media({
                        title: '<?php esc_html_e( 'Select File', 'ayuda-help-desk' ) ?>',
                        multiple: false,
                        button : { text : '<?php esc_html_e( 'Use file', 'ayuda-help-desk' ) ?>' }
                    });
 ...
 */
add_action( 'customize_register', 'registerGASFrameworkOptionFileUploadControl', 1 );
function registerGASFrameworkOptionFileUploadControl() {
    class GASFrameworkOptionFileUploadControl extends WP_Customize_Control {
        public $description;
        public $caption;

        public function render_content() {
            GASFrameworkOptionFile::createUploaderScript();
...
if ( ! function_exists( 'tf_file_upload_option_customizer_get_value' ) ) {
     ...
     *
     * @see GasFrameworkOptionUpload->preLivePreview().
     */
