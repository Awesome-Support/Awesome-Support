...
class GASFrameworkOptionMulticheck extends GASFrameworkOption {
	...
	public function display() {
		...
		if ( ! empty( $this->settings['select_all'] ) ) {
			$select_all_label = __( 'Select All', 'ayuda-help-desk' );
	...
	public function load_select_scripts() {
		...
		wp_enqueue_script( 'tf-multicheck-select-all', GASFramework::getURL( '../js/multicheck-select-all.js', __FILE__ ), array( 'jquery' ), GASF_VERSION, true );
 ...
 */
add_action( 'customize_register', 'registerGASFrameworkOptionMulticheckControl', 1 );
function registerGASFrameworkOptionMulticheckControl() {
	class GASFrameworkOptionMulticheckControl extends WP_Customize_Control {
		public $description;
		public $options;
		public $select_all;
