...
class GASFrameworkCustomizer {
	...
	public function loadUploaderScript() {
		wp_enqueue_media();
		wp_enqueue_script( 'tf-theme-customizer-serialize', GASFramework::getURL( '../js/min/serialize-min.js', __FILE__ ) );
		wp_enqueue_style( 'tf-admin-theme-customizer-styles', GASFramework::getURL( '../css/admin-theme-customizer-styles.css', __FILE__ ) );
	}
	...
	public function ajaxGenerateCustomizerCSS() {
		...
		if ( ! current_user_can( 'read') ) {
			wp_send_json_error( array('message' => __('Unauthorized action. You do not have permission to ajax handler for generating CSS.', 'ayuda-help-desk') ), 403);
		}
	 ...
	 * @param GASFrameworkOption $option The option being parsed.
	 *
			...
			if ( typeof localStorage !== 'undefined' ) {
				...
				wp.ajax.send( 'tf_generate_customizer_css', {
				    success: function( data ) {
						...
						// Add the modified CSS GAS has generated from the preview values.
