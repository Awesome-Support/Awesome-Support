 ...
 *
 * Note: Any changes made here should also be made in the mumei_ayuda_fields_in_ai_tab() function.
 *
 *
 * @since 3.3.5
 */
...
<div class="wpas-custom-fields wpas-ticket-addl-parties-mb">
	...
	do_action( 'mumei_ayuda_mb_details_before_ticket_addl_parties' );
	...
	if ( isset( $options['multiple_agents_per_ticket'] ) && true === boolval( $options['multiple_agents_per_ticket'] ) ) {
		...
		echo esc_html__( 'Note: Email notifications to these agents will be under the name of the PRIMARY agent.', 'ayuda-help-desk' );

		// get id for additional support agents if they are already on the ticket...
		$secondary_staff_id = mumei_ayuda_get_cf_value( 'secondary_assignee', get_the_ID() );
		$tertiary_staff_id = mumei_ayuda_get_cf_value( 'tertiary_assignee', get_the_ID() );
		...
		If ( ! empty( $tertiary_staff_id) ) {
			...
			?> <label for="wpas-secondary-assignee"><strong data-hint="<?php esc_html_e( 'First additional agent who has an interest this ticket', 'ayuda-help-desk' ); ?>" class="hint-left hint-anim"><?php esc_html_e( 'Additional Support Staff #1', 'ayuda-help-desk' ); ?></strong></label><?php

			if ( mumei_ayuda_get_option( 'support_staff_select2_enabled', false ) ) {

				$staff_atts = array(
					'name'      => 'mumei_ayuda_secondary_assignee',
				...
				echo wp_kses(mumei_ayuda_dropdown( $staff_atts, "<option value='$secondary_staff_id' selected='selected'>$secondary_staff_name</option>" ), mumei_ayuda_dropdown_allowed_html_tags());
			} else {
				echo wp_kses(mumei_ayuda_users_dropdown( array(
					'cap'		=> 'edit_ticket',
				...
				) ), mumei_ayuda_dropdown_allowed_html_tags());
			}
			// Display dropdown for tertiary staff
			?><label for="wpas-tertiary-assignee"><strong data-hint="<?php esc_html_e( 'Second additional agent who has an interest this ticket', 'ayuda-help-desk' ); ?>" class="hint-left hint-anim"><?php esc_html_e( 'Additional Support Staff #2', 'ayuda-help-desk' ); ?></strong></label><?php

			if ( mumei_ayuda_get_option( 'support_staff_select2_enabled', false ) ) {
				$staff_atts = array(
					'name'      => 'mumei_ayuda_tertiary_assignee',
				...
				echo wp_kses(mumei_ayuda_dropdown( $staff_atts, "<option value='$tertiary_staff_id' selected='selected'>$tertiary_staff_name</option>" ), mumei_ayuda_dropdown_allowed_html_tags());
			} else {
				echo wp_kses(mumei_ayuda_users_dropdown( array(
					'cap'		=> 'edit_ticket',
					'orderby'	=> 'display_name',
	...
	if ( isset( $options['show_third_party_fields'] ) && true === boolval( $options['show_third_party_fields'] ) ) {
		echo esc_html_e( 'Note: These fields are notational only. They do not participate in notifications!', 'ayuda-help-desk' );
		?></strong><hr /><?php
	...
	do_action( 'mumei_ayuda_mb_details_after_addl_parties' );
	?>
</div>