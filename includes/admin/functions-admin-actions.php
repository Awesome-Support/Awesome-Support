<?php
/**
 * @package   Ayuda – Help Desk/Admin/Functions/Actions
...
function admin_head_post_editing() {
	...
	if( 'ticket' === $post_type ) {
		$can = mumei_ayuda_can_view_ticket( $post_ID );
		...
		if( $can ) {
			...
			mumei_ayuda_write_log('security', 'A logged in user attempted to access a ticket without the necessary permissions. ' . 'Ticket id: ' . (string) $post_ID . ', Logged In user ID: ' . (string) $current_user->ID ) ;
...
add_action( 'mumei_ayuda_do_admin_close_ticket', 'mumei_ayuda_admin_action_close_ticket' );
/**
 * Close a ticket
 *
 * @since 3.3
 ...
 * @return void
 */
function mumei_ayuda_admin_action_close_ticket( $data ) {
...
add_action( 'mumei_ayuda_do_admin_open_ticket', 'mumei_ayuda_admin_action_open_ticket' );
/**
 * (Re)open a ticket
 *
 * @since 3.3
 ...
 * @return void
 */
function mumei_ayuda_admin_action_open_ticket( $data ) {
	...
	mumei_ayuda_reopen_ticket( $post_id );
...
add_action( 'mumei_ayuda_do_admin_trash_reply', 'mumei_ayuda_admin_action_trash_reply' );
/**
 * Trash a reply
 *
 * @since 3.3
 ...
 * @return void
 */
function mumei_ayuda_admin_action_trash_reply( $data ) {
	 ...
	 * There is no UI provision in Ayuda – Help Desk to restore a reply from the trash 
	 * anyway and no anticipated future need where we would need to.  So no harm 
	 * deleting the attachments permanently here.
	 */
	mumei_ayuda_delete_post_attachments( $reply_id ) ;
	
	/* Now trash/delete the post */
	if ( boolval( mumei_ayuda_get_option( 'permanently_trash_replies', false ) ) ) {
	...
	update_post_meta( $ticket_id, 'mumei_ayuda_reply_was_deleted', '1' ) ;
	
	
	/* Fire the after-delete action hook */
	do_action( 'mumei_ayuda_admin_reply_trashed', $reply_id, $trashed_post, $ticket_id  );
...
add_action( 'mumei_ayuda_admin_reply_trashed', 'mumei_ayuda_log_reply_trashed', 10,3 );
/**
 * Log the original contents of a deleted reply.
 *
 * Action hook: mumei_ayuda_admin_reply_trashed
 *
 * @since 5.2.0
 ...
 * @return void
 */
function mumei_ayuda_log_reply_trashed( $reply_id, $original_reply, $ticket_id ) {
	...
	$x_content = __( 'Reply #%1$s was deleted from ticket #%2$s.', 'ayuda-help-desk' );
	...
	mumei_ayuda_log_edits( $original_reply->post_parent, sprintf( $x_content, (string) $reply_id, (string) $original_reply->post_parent  ), $reply_contents_to_log );
