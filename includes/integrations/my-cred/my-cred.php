 ...
 class WPAS_MY_CRED {
	...
	public function after_close_ticket($ticket_id, $update, $user_id) {
		...
		if ( function_exists('mycred_add') ) {
			...
			if ( false == wpas_is_agent( $user_id ) && !empty( wpas_get_option('myCRED_agent_point_type' ) ) && true == wpas_get_option('myCRED_agent_gets_points_user_close' )  ) {
				...
				if ( $agent_id ) {
					mycred_add( $ticket_id, $agent_id, wpas_get_option('myCRED_agent_points_ticket_close'), 'Agent gets points when user closed ticket #' . (string) $ticket_id, $ticket_id, '', wpas_get_option('myCRED_agent_point_type') );				
				}
	...
	public function after_reply_ticket($reply_id, $data) {
		...
		if ( function_exists('mycred_add') ) {
			...
			if ( true == wpas_is_agent( $user_id ) && !empty( wpas_get_option('myCRED_agent_point_type' ) ) ) {
				mycred_add( $ticket_id, $user_id, wpas_get_option('myCRED_agent_points_ticket_reply'), 'Points for agent replying to ticket #' . (string) $ticket_id, $ticket_id, '', wpas_get_option('myCRED_agent_point_type') );
			}
			
			// Add points for user replying to a ticket
			if ( false == wpas_is_agent( $user_id ) && !empty( wpas_get_option('myCRED_user_point_type' ) ) ) {
				mycred_add( $ticket_id, $user_id, wpas_get_option('myCRED_user_points_ticket_reply'), 'Points for user replying to ticket #' . (string) $ticket_id, $ticket_id, '', wpas_get_option('myCRED_user_point_type') );
			}
