<?php
class MUMEI_AYUDA_Test_Custom_Field extends WP_UnitTestCase {
	 ...
	 * Text custom field.
	 *
	 * @var MUMEI_AYUDA_Custom_Field $text_field
	 */
	public $text_field;
	...
	function setUp() {
		 ...
		 * Add a text field.
		 */
		mumei_ayuda_add_custom_field( 'my_test_field', array(
			'field_type'    => 'text',
		 ...
		 * Add a test ticket.
		 */
		$this->ticket_id = mumei_ayuda_insert_ticket( array(
		 ...
		 */
		$this->text_field = new MUMEI_AYUDA_Custom_Field( 'my_test_field', $this->fields['my_test_field'] );

	}

	public function test_get_class_name() {
		$this->assertEquals( 'MUMEI_AYUDA_CF_Text', $this->text_field->get_class_name() );
	}

	public function test_mumei_ayuda_add_custom_field() {
		$this->assertArrayHasKey( 'my_test_field', $this->fields );
	}

	public function test_get_field_id() {
		$this->assertEquals( 'mumei_ayuda_my_test_field', $this->text_field->get_field_id() );
	}

	public function test_get_field_id_save() {
		$this->assertEquals( '_mumei_ayuda_my_test_field', $this->text_field->get_field_id( true ) );
	}
	...
	public function test_update_value() {
		...
		$meta_value = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->postmeta WHERE meta_key = '%s' AND post_id = '%d'", '_mumei_ayuda_my_test_field', $this->ticket_id ) );
	...
	public function test_get_field_value() {
		update_post_meta( $this->ticket_id, '_mumei_ayuda_my_test_field', 'hello' );
		$this->assertEquals( 'hello', $this->text_field->get_field_value( '', $this->ticket_id ) );
		$this->assertEquals( 'hello', mumei_ayuda_get_cf_value( 'my_test_field', $this->ticket_id, '' ) );
	}
