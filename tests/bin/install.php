...
if ( $installed ) {
	echo "Deleting previous test data…" . PHP_EOL;
	$reset = $wpdb->query( 'DROP DATABASE ' . DB_NAME . ";" );
	if ( $reset ) {
		echo "[OK] Previous data deleted.\n" . PHP_EOL;
	} else {
		echo "[ERROR] Previous data could not be deleted. Tests results might not be accurate.\n" . PHP_EOL;
	}
}

echo "Installing fresh database…" . PHP_EOL;

$wpdb->query( 'SET storage_engine = INNODB;' );
$database = $wpdb->query( 'CREATE DATABASE ' . DB_NAME . ";" );
$wpdb->select( DB_NAME, $wpdb->dbh );

if ( $database ) {
	echo "[OK] Fresh database installed.\n" . PHP_EOL;
} else {
	echo "[ERROR] Fresh database could not be installed. Tests might fail.\n" . PHP_EOL;
}

if ( file_exists( WP_TESTS_VERSION_FILE ) ) {
